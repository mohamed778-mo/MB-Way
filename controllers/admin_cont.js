...
const remove_verifyEmployeeEmail = async (req, res) => {
  try {
   ...
   await user.save();
    res.status(200).send("Email is Not_verified!");
  } catch (e) {
    res.status(500).send(e.message);  
  }
};
  ...
  try {
    ...
    if (!user_data.isAdmin ) {
      ...
      await employee_data.save()
       ...
       res.status(200).send(`${employee_data.name} is now a manager !!`);
  ...
  try {
    ...
    if (!user_data.isAdmin ) {
      ...
      await employee_data.save()
       ...
       res.status(200).send(`${employee_data.name} is now a not_manager !!`);
  ...
  try {
    ...
    if (!user_data.isAdmin ) {
      ...
      await employee_data.save()
       ...
       res.status(200).send(`${employee_data.name} is now BLOCKED !!`);
  ...
  try {
    ...
    if (!user_data.isAdmin ) {
      ...
      await employee_data.save()
       ...
       res.status(200).send(`${employee_data.name} is now a NOT_BLOCKED !!`);
...
const delete_all_taskings= async(req,res)=>{
 ...
 res.status(200).send('All Tasking deleted successfully!')
